...
describe('fetchCommand', () => {
    ...
    it('fetches all configured sources and writes snapshots', async () => {
        ...
        // Normalise separators: fetchCommand resolves the output dir, so the
        // mocked POSIX '/repo' comes back drive-qualified and backslashed on
        // Windows. Comparing on sep keeps this assertion true on both.
        const firstPath = (result.paths[0] ?? '').split(sep).join('/');
        expect(firstPath).toContain('/repo/data/raw/norse-runes.raw.txt');
    });
