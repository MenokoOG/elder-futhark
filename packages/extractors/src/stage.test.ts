...
describe('extractSourceRecords', () => {
    it('extracts records with preserved provenance fields', () => {
        ...
        expect(records[0]?.references[0]).toMatchObject({
            sourceUrl: 'https://norse-mythology.org/runes/the-origins-of-the-runes/',
            classification: 'reference_like',
    ...
    it('does not put the source classification into the content tags', () => {
        // This leak made every deity come out with domains: ["reference_like"].
        const records = extractSourceRecords({
            source: {
                id: 'norse-god-odin',
                url: 'https://norse-mythology.org/gods-and-creatures/odin/',
                classification: 'reference_like'
            },
            snapshot: {
        ...
        expect(records[0]?.references[0]?.classification).toBe('reference_like');
    });
    ...
    it('routes norse subpage ids to matching extractor families', () => {
        ...
        expect(runeRecords[0]?.kind).toBe('generic_page');
